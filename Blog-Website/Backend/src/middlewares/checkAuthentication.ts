...
const checkAuthentication = async( req: Request, res: Response, next: NextFunction ) =>
{
    try{
        ...
        if(!token)
        {
            return res.status(401).json( {success: false, message: "Unauthenticated. No token found"  } )
        }
        let decodedUserInfo = await jwt.verify(token, process.env.JWT_SECRET as string )
        
        if(!decodedUserInfo)
        {
            return res.status(401).json({ success: false, message: "Unauthenticated. Invalid token" })
        }   
        //@ts-ignore
