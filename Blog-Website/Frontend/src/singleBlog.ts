...
const totalCommentContainer = document.querySelector(
  ".total-comment-container"
) as HTMLSpanElement;
const blogImageContainer = document.querySelector(
  ".blog-image-container"
) as HTMLImageElement;
const blogDescriptionContainer = document.querySelector(
  ".blog-desc-container"
) as HTMLElement;
const blogCardContainer = document.querySelector(
  ".blog-card-container"
) as HTMLElement;
...
window.addEventListener("load", async (event) => {
                ...
                <div class=" mb-10">
                    <figure class="">
                        <a href= '/singleblog?blog_id=${
                          randomBlogInfo[index].blog_id
                        }'>
                        <img src=${encodeURI(
                          randomBlogInfo[index].image
                        )} alt="" class="h-[80%] w-[100%] object-fill opacity-70 hover:opacity-100">
                        </a>
                    </figure>
                    <h2 class="font-bold text-4xl mt-1 sm:m-7">${
                      randomBlogInfo[index].title
                    }</h2>
                    <div class="flex flex-row gap-7 mt-2 text-xl uppercase ">
                        <span class="text-gray-500 uppercase">${convertIsoToFormattedData(
                          randomBlogInfo[0].createdAt
                        )} </span>
                        <span class="text-black">${
                          randomBlogInfo[index].Comments.length
                        } Comments</span>
                    </div>
                </div>
            </div>
  ...
  authorMugshot.innerHTML = `
    <img src=http://${encodeURI(
      adminInfo.image
    )} alt="author" class="author-mugshot w-full h-full rounded-full">
    `;
  ...
  if (blogInfo.Comments.length > 0) {
    for (let index = 0; index < totalComments; index++) {
      commentsContainer.innerHTML += `
           ...
           <span class="capitalize text-3xl font-bold">${
             blogInfo.Comments[index].User?.name
           }</span>
           <span class="uppercase text-xs tracking-tight">${convertIsoToFormattedData(
             blogInfo.Comments[index].createdAt
           )}</span>
           </div>
