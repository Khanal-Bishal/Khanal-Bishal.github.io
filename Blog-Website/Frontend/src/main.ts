...
const blogImageContainer = document.querySelector(
  ".blog-image-container"
) as HTMLElement;
const heading = document.querySelector(".heading") as HTMLElement;
const commentContainer = document.querySelector(
  ".total-comment-container"
) as HTMLElement;
const sblogImageContainer = document.querySelector(
  ".single-blog-first-image"
) as HTMLElement;
const readMore = document.querySelector(".read-more") as HTMLElement;
const youMayLikeContainer = document.querySelectorAll(
  ".youMayLike-container"
) as NodeListOf<Element>;
const secondBlogHeading = document.querySelector(".s-heading") as HTMLElement;
const secondCommentContainer = document.querySelector(
  ".stotal-comment-container"
) as HTMLElement;
const secondBlogImageContainer = document.querySelector(
  ".single-blog-second-image"
) as HTMLElement;
const secondReadMore = document.querySelector(
  ".second-read-more"
) as HTMLElement;
  ...
  blogImageContainer.innerHTML = `
      <div class=" relative cursor mb-3 w-[100%] mx-1 sm:w-[60%] ">
        ...
        <div class="absolute top-[50%] left-[30%] translate-y-[-50%] translate-x-[-30%] text-white ">
          <h2 class="font-bold text-4xl capitalize w-[85%]"> ${
            randomBlogs[0].title
          } </h2>
          ...
          <button class="border-none bg-red-300 uppercase p-4 mt-7 text-xl font-semibold w-[150px]   text-center "> 
             <a href='/singleblog?blog_id=${
               randomBlogs[0].blog_id
             }' >Read more </a>
            </button> 
        </div>
      </div>
      ...
      <div class="w-[100%] sm:w-[40%] ">
        <div class=" relative cursor mb-3 w-[100%] mx-1  ">
          ...
          <img src=${encodeURI(
            randomBlogs[1].image
          )} alt="blogImage " class="w-[100%] h-[250px]  aspect-square object-cover  sm:h-[200px] ">
          <div class="absolute inset-0 bg-black opacity-65 hover:opacity-45"></div>
          </figure>

          <div class="absolute top-[50%] left-[50%] translate-y-[-50%] translate-x-[-50%] text-white ">
            <h2 class="font-bold text-4xl capitalize  text-center">${
              randomBlogs[1].title
            }</h2>
            <div class="flex justify-center font-semibold gap-10 uppercase mt-5">
              <p class=" mt-5 text-sm text-gray-200"> In Blog </p>
              <p class=" mt-5 text-sm"> ${convertIsoToFormattedData(
                randomBlogs[1].createdAt
              )} </p>
            </div>
          </div>
        </div>
  ...
  youMayLikeContainer.forEach(async (container) => {
    ...
    for (let index = 0; index < 3; index++) {
      ...
      <a href='./singleblog?blog_id=${randomBlogs[index].blog_id}'>
        <h2 class="font-bold text-2xl mt-7 cursor">${
          randomBlogs[index].title
        } </h2>
      </a>
      ...
      <span class="text-black">${
        randomBlogs[index].Comments.length
      } comment</span>
      </div>
      </div>
      `;
